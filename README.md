<div align="center">

# 🚀 Hi, I'm Kanap Pratik Popat  

</div>

---

## 🧠 About Me  

Aspiring Software Engineer with strong foundations in Java, DSA, and full-stack development. Experienced in building scalable web applications using Spring Boot, React.js, Node.js, and relational databases. Eager to apply problem-solving skills and contribute to impactful software projects.

---

# 🛠️ Tech Stack

## 💻 Languages
<p>
<img src="https://skillicons.dev/icons?i=java,cpp,cs,js" />
</p>

## ⚙️ Backend
<p>
<img src="https://skillicons.dev/icons?i=spring,nodejs,express,dotnet" />
</p>

## 🎨 Frontend
<p>
<img src="https://skillicons.dev/icons?i=react,html,css,bootstrap" />
</p>

## 🗄️ Database
<p>
<img src="https://skillicons.dev/icons?i=mysql,mongodb" />
</p>

## ☁️ Cloud & Tools
<p>
<img src="https://skillicons.dev/icons?i=git,github,vscode" />
</p>

---

# 🎓 Academic Details

## 🎓 PG-DAC (Aug 2025 Batch)  
**Total: 200 / 320**

---

## 🎓 B.Tech – Computer Science & Engineering  
**Institute:** Tatyasaheb Kore Institute of Engineering & Technology, Warananagar, Kolhapur  
**University:** Shivaji University, Maharashtra  
**Year:** 2025  
**Percentage:** 69.45%  

---

## 🎓 XII – Science (CBSE)  
Kendriya Vidyalaya (B.E.G), Yerawada, Pune  
Year: 2021  
Percentage: 64.4%  

---

## 🎓 X – CBSE  
Kendriya Vidyalaya (B.E.G), Yerawada, Pune  
Year: 2019  
Percentage: 68.6%  

---

# 🚀 Academic Projects

## 🥗 NutriFit – Fitness & Diet Planning Platform

**Platform:** ASP.NET Core, C#, React.js, MySQL, Git, AWS  
**Duration:** 1 Month  

✔ Developed full-stack fitness tracker & diet planner  
✔ Built responsive React UI for cross-device compatibility  
✔ Designed feature-rich admin dashboard  
✔ Integrated MySQL with optimized data access  
✔ Deployed on AWS ensuring scalability & maintainability  

🔗 Repository:  
https://github.com/harode12/NutriFit  

---

## 🎓 Academix – Attendance & Performance Management System  

**Platform:** React.js, Node.js, Express.js, MongoDB, REST API  
**Duration:** 4 Months  

✔ Role-based dashboards (Student/Teacher)  
✔ Built secure REST APIs with Express  
✔ Implemented attendance tracking & analytics  
✔ Automated alerts for defaulters  
✔ Responsive and user-friendly UI  

🔗 Repository:  
https://github.com/pratikkanap/Academix  

---

# 💼 Internship Experience

**Java Developer Intern**  
Technocipher IT Solutions, Pune  

Worked on backend modules, API development, and database integration.

---

# 📊 GitHub Analytics

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=pratikkanap&show_icons=true&theme=tokyonight&hide_border=true" />
<img height="170" src="https://github-readme-streak-stats.herokuapp.com/?user=pratikkanap&theme=tokyonight&hide_border=true" />

</div>

---

# 🎯 Career Objective  

Seeking an opportunity as a Software Engineer / Java Backend Developer / ASP .NET Developer / Full-Stack Developer where I can contribute to scalable enterprise applications and grow in backend architecture.

---

# 🌐 Connect With Me  

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Kanap_PratiK-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/kanap-pratik/)

</div>

---

<div align="center">

🔥 *“First, solve the problem. Then, write the code.”*

</div>
